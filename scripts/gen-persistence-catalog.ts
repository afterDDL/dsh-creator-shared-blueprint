...
export interface LogEventEntry {
  ...
  name: string
  /** npm package whose declaration supplies the event semantics in this build. */
  owner: string
...
export function collectLogEvents(scanRoot: string = root): LogEventEntry[] {
  ...
  for (const rel of globSync('packages/*/*/src/**/*.ts', { cwd: scanRoot }).map(s => s.split(sep).join('/')).sort()) {
    ...
    const packageOwner = packageNameFor(rel, scanRoot)
    if (packageOwner === null) {
      violations.push(`SessionEventMap source ${rel} has no readable owning package manifest.`)
      continue
    }
    ...
    for (const { decl, topLevel } of sessionEventMapDecls(sf)) {
      ...
      if (topLevel) {
        ...
        if (packageOwner !== SESSION_PACKAGE) {
          violations.push(`top-level interface SessionEventMap (${declSrc}) is outside ${SESSION_PACKAGE} (package ${packageOwner}). Rename the interface, or contribute events via declare module '${SESSION_TYPES_MODULE}'.`)
          continue
        }
      ...
      for (const member of decl.members) {
        ...
        entries.push({ name, owner: packageOwner, scope: name.split('/')[0] ?? name, payload, declaration, doc, source: src })
      }
    }
  }
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  const entries = [...events].sort((left, right) => left.name.localeCompare(right.name))
  return [
    '/**',
    ...
    ...entries.map(event => `  [${JSON.stringify(event.name)}, ${JSON.stringify(event.owner)}],`),
    '])',
    '',
