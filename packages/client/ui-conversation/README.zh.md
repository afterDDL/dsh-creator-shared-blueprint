...
`ctx.conversation.interactions` 是按 source 寻址的内存注册表，用于外部 `PendingInteraction` carrier。生产方会替换一个 source Session 当前展示的确切 carrier；`ConversationRoot` 先保留该 source 的原生 wait，再追加外部 carrier，且只在原生 wait 具有相同 owning Session 与 carrier key 时抑制该外部 carrier。该注册表只授予展示权，不转移响应归属：标准 composer chain 通过原生 selector 渲染每项 interaction，并通过原 carrier 回答，因此 child Session 保留 RPC 关联与结算，原生 question 或 approval domain 仍负责响应编码。清空 source list 或 forget 其 store 只会移除这份展示。
