...
Sidebar shell plugin: the wordmark, New Session action, layout-owned collapse control, additive navigation sections, scroll-aware region seat, and bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`sidebar.navigation.section` is an ordered, root-scoped list above the workspace browser. A client plugin can wait for its declaration through `ctx.slots.inject` and register a stable `id` plus optional `order`; multiple independent sections coexist instead of replacing the Workspace browser or the whole sidebar. Each entry receives `SidebarNavigationSectionProps`, including the shell-derived `wide` flag and an `expandSidebar()` callback that is effective only on the rail. The entry decides whether to render a rail control or nothing while narrow. Disposing its registration removes only that section.

`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared navigation, workspace, settings, and footer-action child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.

During a live collapse, the shell holds the expanded content at its current width while it fades out for 150ms. The four upper controls—the shell toggle and New Session plus add and search rendered through `sidebar.workspaces`—then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. Navigation-section entries remain mounted and receive the settled `wide` value, so their own component controls whether and how they occupy the rail. The bottom-pinned `sidebar.settings` control shares the fade timing but has no horizontal translation. A page that starts collapsed renders the rail statically, and reduced-motion mode disables both transitions.
