...
`sidebar.navigation.section` 是位于 workspace 浏览器之上的有序 root 作用域列表。Client 插件可以通过 `ctx.slots.inject` 等待该声明，并使用稳定 `id` 和可选 `order` 注册；多个独立 section 可以并存，而不是替换 Workspace 浏览器或完整侧边栏。每个 entry 都会收到 `SidebarNavigationSectionProps`，其中包含外壳派生的 `wide` 标记和只在轨道状态生效的 `expandSidebar()` 回调。Entry 自行决定窄态时渲染轨道控件还是不渲染。释放注册只移除该 section。

`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 navigation、workspace、settings 和 footer-action 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。

实时收起时，外壳会把展开内容固定在当前宽度，并用 150ms 将其淡出。随后，上方四个控件——外壳的侧栏切换与新建会话，以及通过 `sidebar.workspaces` 渲染的添加和搜索——共用一次 150ms 的淡入和 49px 左移，在布局的 300ms 栏滑动结束时一起进入 56px 轨道；每个 36px 控件盒都会沿同一条路径到达轨道左侧 10px 的内边距。Navigation-section entry 保持挂载并收到稳定后的 `wide` 值，因此它们自己的组件会决定是否以及如何占用轨道。固定在底部的 `sidebar.settings` 控件只共用淡入时序，不发生横向位移。页面初始即为收起状态时会静态渲染轨道；减少动态效果模式会禁用两段过渡。
