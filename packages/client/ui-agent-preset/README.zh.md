...
chip 以部署默认值打开，其选择是**暂存**的——该界面先于它要应用到的会话存在。暂存值会在某个会话成为当前会话且仍为空白时抵达该会话；这既覆盖工作区连接新建的会话，也覆盖它复用的那个空白会话，而搭 `sessions.create` 的便车会漏掉后者。Host 重组该空白会话期间，seat 会设置一个限定于该会话的 composer block；渲染出的禁用状态与 conversation 发送路径都会执行它，并且只有 `agentPresets.select` 返回已提交的 preset identity 后才解除。切换失败会保留输入阻塞并给出可重试说明，不允许首条 prompt 在 fallback composition 上运行。暂存值一经使用即被清空，因此下一个新会话重新以默认值打开——与它旁边的工作区选择器完全一致。

`AgentPresetSessionIntent` 会把同一项由 seat 持有的暂存操作提供给其他 Client surface。Creator 入口通过它暂存 `cordis`；它不会修改当前 Session，也不会在 Host 回显已提交 preset 之前声称新 Session 已属于 Creator。
