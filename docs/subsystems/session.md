...
In-repository declarations enter the generated first-party vocabulary together with their npm package owner. A package may register that same owner so identical startup code works when it is bundled or installed out of tree; another owner and duplicate live registrations reject. An out-of-tree plugin registers each required merged type through `ctx.sessions.eventTypes.register({ type, owner })` during plugin application, before persistence decodes a Session carrying it. The registration follows the plugin fiber: removal or restart removes it until the plugin registers again, and a missing registration keeps the required-event refusal fail-closed. A payload-incompatible plugin version must use a new event type instead of claiming compatibility by re-registering the old type.
...
Source: [`packages/core/session/src/index.ts:859`](../../packages/core/session/src/index.ts)
