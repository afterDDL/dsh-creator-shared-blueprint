...
- `@deepseek-ai/dsh-blueprint-adapter` — 需要 `agentPresets` · `systemPrompt` · `agents` · `sessions` · `tools` · `skills` · `subagents`（[`packages/preset/blueprint-adapter/src/index.ts`](../packages/preset/blueprint-adapter/src/index.ts)）
