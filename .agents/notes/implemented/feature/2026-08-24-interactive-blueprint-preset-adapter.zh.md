...
静态 Demo 入口会挂载同一套 Web Client shell、slot 注册、controller、bootstrap seed 与 Blueprint-only fixture 行为，而不会启动 DSH Host。它的内存 connection 只实现已验收 walkthrough 实际使用的 Session、workspace、model、interaction 与 projection 读写；不受支持的 Remote 调用会失败，不会扩展成平行 backend。构建会生成现有 Client UI 所需的 Commands browser descriptor，在可配置的仓库 base path 下输出资源，并排除 source map。它不会建立 HTTP、WebSocket、provider、persistence、filesystem、Skill runtime、Subagent runtime 或 Agent loop 连接。
