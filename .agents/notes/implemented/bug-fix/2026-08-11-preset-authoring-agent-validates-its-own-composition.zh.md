...
[仅供 Creator 使用的固定 preset authoring Consumer](2026-08-30-creator-only-preset-authoring-consumer.md)取代了下文所述的临时自挂载 probe。Creator 必须通过真实 standing mount 而非发现阶段健康状态完成校验，这项决定仍然有效。
...
原始交付方式遵循 `cordis_mount` 的文档：一个声明 `inject: ['agentPresets', 'tools']` 的临时插件为组装它的 agent 注册工具。挂载只返回自身确认，已注册工具则在下一步把服务答案交给模型。skill 逐字附带了该插件。`agentPresets` 可通过生成的 `cordis_inspect what:"api"` 目录及完整 JSDoc 获取；sandbox facade 按 `fiber.inject` 而非白名单放行服务，因此这条路径不需要为 skill 增加特例。

`copy(from, id, name)` 仍是固定 Consumer 下层的创作写入手段，取代 shell 复制：它校验 id、拒绝任何根已提供的 id、失败时回滚、重写副本的 `preset.yml`，并在宿主侧运行而无需 sandbox 升级。之后编辑 `agent.cordis.yml` 仍在 Session 工作区之外，因此 sandbox 升级说明仍适用于该步骤。

「某行是否发布服务」改由 `cordis_inspect what:"services"` 回答，它会给出每个存活服务的持有 fiber。

指导保留 `${DSH_HOME:-$HOME/.dsh}/.agent-presets/` 作为「我的 preset 在哪」的答案，同时让 agent 实际读取或编辑的路径通过 roster 的 `list()` 或 `resolve()` 操作取得。写出该路径对人讲是对的，喂给文件工具是错的：部署可以配置其他根目录，而 `list()` 无法揭示一个尚且为空的用户根。
...
下表每一行都由启动已发布的 Web 组装、并在由 `cordis` 组装出的 agent 上执行 roster standing mount 得出——全程无模型参与。固定 Consumer 继续直接委托该操作完成 validation，不转换失败。
...
原始 `cordis_mount` 片段经 Tool registry 执行：它成功挂载，在下一次读取目录时向组装该 agent 的目录暴露 `preset_check`，对有效 preset 返回 `mounted OK`，对无效 preset 返回挂载拒绝。fixed Consumer Note 负责记录替代机制的首请求验证。
...
**增加一等的 preset validation Tool。** 组合出的路径已经存在，且由 `cordis_mount` 自身文档记载；当时，专用 Tool 会给一个意在无需专用 Tool 即可够到 runtime 的 preset 增加面向模型的行。首请求 schema 稳定性后来成为要求，fixed Consumer 决定据此取代了这项 delivery 取舍。
...
- standing-mount validation 规则不依赖 Creator 如何访问它。固定 Consumer 取代临时 Tool 注册，同时保留相同的 `standingKeyFor()` 结果与清理行为。
